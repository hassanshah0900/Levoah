import { pgTable, foreignKey, pgPolicy, bigint, integer, jsonb, timestamp, text, unique, smallint, boolean, primaryKey, pgView, json, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const collectionType = pgEnum("collection_type", ['manual', 'automatic'])
export const matchType = pgEnum("match_type", ['and', 'or'])
export const productType = pgEnum("product_type", ['glasses', 'accessories', 'lenses'])
...
export const productVariants = pgTable("product_variants", {
	...
	pgPolicy("allow delete access on product_variants based on role", { as: "permissive", for: "delete", to: ["public"], using: sql`authorize('product_variants.delete'::text)` }),
	pgPolicy("allow insert access on product_variants based on role", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("allow read access on product_variants based on role", { as: "permissive", for: "select", to: ["public"] }),
	pgPolicy("allow update access on product_variants based on role", { as: "permissive", for: "update", to: ["public"] }),
]);
...
export const categories = pgTable("categories", {
	...
	pgPolicy("allow select access to all users", { as: "permissive", for: "select", to: ["public"], using: sql`true` }),
	pgPolicy("allow delete access on categories to only admin", { as: "permissive", for: "delete", to: ["public"] }),
	pgPolicy("allow update access on categories to only admin", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("allow insert access on categories to only admin", { as: "permissive", for: "insert", to: ["public"] }),
]);
...
export const conditions = pgTable("conditions", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "conditions_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
...
export const orders = pgTable("orders", {
	...
	pgPolicy("allow access to everyone", { as: "permissive", for: "insert", to: ["public"], withCheck: sql`true`  }),
...
export const colors = pgTable("colors", {
	id: integer().primaryKey().generatedByDefaultAsIdentity({ name: "colors_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	name: text().notNull(),
	type: text().notNull(),
}, (table) => [
...
export const orderItems = pgTable("order_items", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "order_items_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	quantity: integer(),
	...
	pgPolicy("allow access to everyone", { as: "permissive", for: "all", to: ["public"], using: sql`true`, withCheck: sql`true`  }),
	pgPolicy("allow access on order items", { as: "permissive", for: "insert", to: ["public"] }),
]);
...
export const products = pgTable("products", {
	...
	pgPolicy("allow insert access on products based on role", { as: "permissive", for: "insert", to: ["public"] }),
]);
...
export const collectionProducts = pgTable("collection_products", {
	...
	primaryKey({ columns: [table.collectionId, table.productId], name: "collection_products_pkey"}),
]);
...
export const glasses = pgView("glasses", {	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	...
	category: jsonb(),
}).as(sql`SELECT p.id, p.title, p.description, p.created_at, p.published, p.slug, p.attributes, p.product_type, to_jsonb(c.*) AS category FROM products p LEFT JOIN product_categories pc ON pc.product_id = p.id LEFT JOIN categories c ON pc.category_id = c.id`);
