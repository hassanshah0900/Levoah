...
export const productVariantsWithImages = pgView("product_variants_with_images", {	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	...
	productId: bigint("product_id", { mode: "number" }),
	imageUrl: text("image_url"),
}).as(sql`SELECT pv.id, pv.price, pv.quantity_in_stock, pv.attributes, pv.created_at, pv.product_id, i.path AS image_url FROM product_variants pv LEFT JOIN images i ON i.variant_id = pv.id AND i.product_id = pv.product_id`);